### Derive the aggregate MNase count matrix for Broggard nucleosomes on chrIV as the reference nucleosome matrix
broggard_nuc.df=read.table('../Data/Broggard_sacCer3_liftover.bed',header = F)
chr4_nuc.df=subset(broggard_nuc.df,V1=='chrIV')
...
for (l in 1:length(sample_filenames)){
  ...
  mid.df=subset(mid.df,chr=='chrIV')
  chr4_counts.m=matrix(0,250,601)
  for (i in 1:nrow(chr4_nuc.df)){
    counts.m=matrix(0,250,601)
    mid_nuc.df=subset(mid.df,mid>=(chr4_nuc.df$V2[i]-300)&mid<=(chr4_nuc.df$V2[i]+300))
    for (size in 1:250){
      mid_nuc_size.df=subset(mid_nuc.df,length==size)
      counts.m[size,]=hist(mid_nuc_size.df$mid,seq(chr4_nuc.df$V2[i]-300,chr4_nuc.df$V2[i]+300+1,1),plot = F,right = F)$counts
    }
...
saveRDS(counts_matrix.l,file='chr4_broggard_aggregate_matrix.l.RDS')

### Calculate nucleosome 2D correlation score with reference Broggard nucleosome matrix
nuc_kernel<-function(mu,Sigma,x_width=151,ybot=20,ytop=250){
  library(mvtnorm)
...
calculate_nuc_cross_cor_with_gr<-function(filename,chr,start_pos,end_pos,ybot=20,ytop=250,x_width=151,whole_genome='no'){
  library(GenomicRanges)
  library(Rsamtools)
  library(reticulate)
  ...
  broggard_matrix.l=readRDS('chr4_broggard_aggregate_matrix.l.RDS')
  ...
  bivn.m<-nuc_kernel(mu=c(mean(y.t),mean(x.t)),Sigma = matrix(c(var(y.t)/16,0,0,var(x.t)/4),2))
  ...
  if (whole_genome=='no') {
    ...
    write.table(cross_cor.df,file=paste0(filename,'_',chr,'_',start_pos,'_',end_pos,'_nuc_cross_cor.txt'),row.names = F)
    ...
    write.table(cross_cor.df,file=paste0(filename,'_nuc_cross_cor.txt'),row.names = F)
    ...
    for (ch in 1:16){
      ...
      for (s in 1:(length(start_seq)-1)){
        ...
        write.table(cross_cor.df,file=paste0(filename,'_nuc_cross_cor.txt'),row.names = F,append = T,col.names = F)
      }
...
for (filename in sample_filenames){
  calculate_nuc_cross_cor_with_gr(filename,whole_genome = 'yes')
}
